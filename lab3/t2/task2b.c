...
void fixfile(){
    ...
    char viroffset[64];
    fgets(viroffset,64,stdin);
    int signitureOffset;
    sscanf(viroffset,"%d",&signitureOffset);
    printf("Enter the size of the virus signature:\n");
    char virSize[64];
    fgets(virSize,64,stdin);
    int signitureSize;
    sscanf(virSize,"%d",&signitureSize);
    kill_virus(*argvplus1,signitureOffset,signitureSize);
}
